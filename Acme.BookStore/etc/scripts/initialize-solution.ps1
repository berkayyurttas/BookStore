...
$jobs += Start-Job -Name "DbMigrator" -ScriptBlock {
    $ErrorActionPreference = "Stop"
    Set-Location (Join-Path $using:scriptRoot "../../src/Acme.BookStore.DbMigrator")
    dotnet run
...
$jobs += Start-Job -Name "DevCert" -ScriptBlock {
    $ErrorActionPreference = "Stop"
    Set-Location (Join-Path $using:scriptRoot "../../src/Acme.BookStore.HttpApi.Host")
    dotnet dev-certs https -v -ep openiddict.pfx -p d3d727e7-7ec5-4440-91b0-74cb802b232b
